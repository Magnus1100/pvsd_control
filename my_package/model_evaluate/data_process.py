...
# 生成连续的序号
index1 = pd.RangeIndex(start=1, stop=len(copied_aziumth) + 1)
index2 = pd.RangeIndex(start=0, stop=len(copied_aziumth))
copied_interval.index = index2
copied_angle.index = index2

# 创建新的 DataFrame，使用连续的序号作为索引
new_df = pd.DataFrame({'Azimuth': copied_aziumth})
...
normalized_data = mormalized_data(new_df)